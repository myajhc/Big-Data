...
// krate a Kafka connection
const kafka = new Kafka({
	clientId: options.kafkaClientId,
	brokers: [options.kafkaBroker], // one broker, because of the bootstrap
	retry: {
		retries: 0
	}
})
...
// Send tracking message to Kafka whenever a client launches a game
...
function sendResponse(res, html, cachedResult) {
	res.send(`<!DOCTYPE html>
		...
		<head>
			...
			<script>
				function fetchRandomGamess() {
					const maxRepetitions = Math.floor(Math.random() * 200)
					document.getElementById("out").innerText = "Fetching " + maxRepetitions + " random gamess, see console output"
					for(var i = 0; i < maxRepetitions; ++i) {
						const gamesId = Math.floor(Math.random() * ${numberOfGamess})
						console.log("Fetching games id " + gamesId)
						fetch("/gamess/sts-" + gamesId, {cache: 'no-cache'})
					}
				}
			</script>
		</head>
		<body>
			...
			<p>
				<a href="javascript: fetchRandomGamess();">Randomly fetch some gamess</a>
				<span id="out"></span>
			</p>
			${html}
			<hr>
...
// Get list of gamess (from cache or db)
async function getGamess() {
	const key = 'gamess'
	...
	if (cachedata) {
		...
		const data = await executeQuery("SELECT games FROM gamess", [])
		if (data) {
			...
			throw "No gamess data found"
		}
	}
}

// Get popular gamess (from db only)
async function getPopular(maxCount) {
	const query = "SELECT games, count FROM popular ORDER BY count DESC LIMIT ?"
	return (await executeQuery(query, [maxCount]))
		.map(row => ({ games: row?.[0], count: row?.[1] }))
}

// Return HTML for start page
app.get("/", (req, res) => {
	const topX = 10;
	Promise.all([getGamess(), getPopular(topX)]).then(values => {
		const gamess = values[0]
		const popular = values[1]

		const gamessHtml = gamess.result
			.map(m => `<a href='gamess/${m}'>${m}</a>`)
			.join(", ")

		const popularHtml = popular
			.map(pop => `<li> <a href='gamess/${pop.games}'>${pop.games}</a> (${pop.count} views) </li>`)
			.join("\n")

		const html = `
			<h1>Top ${topX} Gamess</h1>		
			<p>
			...
			<p> ${gamessHtml} </p>
		`
		sendResponse(res, html, gamess.cached)
	})
})

// -------------------------------------------------------
// Get a specific games (from cache or DB)
// -------------------------------------------------------

async function getGames(games) {
	const query = "SELECT games, heading, description FROM gamess WHERE games = ?" //////////////////////////////////////////////////////////////////////////////////////
	const key = 'games_' + games
	let cachedata = await getFromCache(key)
	...
	if (cachedata) {
		...
		let data = (await executeQuery(query, [games]))?.[0] // first entry
		if (data) {
			let result = { games: data?.[0], heading: data?.[1], description: data?.[2] }
			console.log(`Got result=${result}, storing in cache`)
			if (memcached)
				await memcached.set(key, result, cacheTimeSecs);
			return { ...result, cached: false }
		} else {
			throw "No data found for this games"
		}
	}
}

app.get("/gamess/:games", (req, res) => {
	...
	}).then(() => console.log(`Sent games=${games} to kafka topic=${options.kafkaTopicTracking}`))
		.catch(e => console.log("Error sending to kafka", e))

	// Send reply to browser
	getGames(games).then(data => {
		sendResponse(res, `<h1>${data.games}</h1><p>${data.heading}</p>` +
